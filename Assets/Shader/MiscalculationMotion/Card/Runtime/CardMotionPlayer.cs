...
namespace Miscalculation.Motion.Card
{
    ...
    /// </summary>
    public sealed class CardMotionPlayer : MonoBehaviour
    {
        [SerializeField] private CardMotionProfile profile = new CardMotionProfile();
        [SerializeField, Min(0.01f)] private float playbackSpeed = 1f;
        private int dustBurstSerial;

        public CardMotionProfile Profile
        {
            get => profile;
            set => profile = value ?? new CardMotionProfile();
        }
            ...
            RectTransform impactVisual = null)
        {
            ...
            float contactDurationMs = Mathf.Clamp(profile.land.durationMs * 0.18f, 24f, 70f);
            float slideDurationMs = Mathf.Max(35f, profile.land.durationMs - contactDurationMs);
            Vector2 impactScale = new Vector2(profile.land.squashX, profile.land.squashY);
            float contactUniformScale = (landStartScale.x + landStartScale.y) * 0.5f;

            // First make the table contact readable: position is locked while the card compresses.
            // The following slide therefore reads as friction on the table instead of unfinished flight.
            yield return Animate(contactDurationMs, "easeOutCubic", (eased, raw) =>
            {
                Vector2 scale = Vector2.LerpUnclamped(landStartScale, impactScale, eased);
                float uniformScale = Mathf.LerpUnclamped(contactUniformScale, 1f, eased);
                if (deformationVisual != null)
                {
                    deformationVisual.localScale = new Vector3(
                        scale.x / Mathf.Max(0.0001f, uniformScale),
                        scale.y / Mathf.Max(0.0001f, uniformScale),
                        1f);
                }
                SetTransform(
                    card,
                    landStart,
                    landStartRotation,
                    deformationVisual != null ? Vector2.one * uniformScale : scale);
            });

            if (profile.land.impactHoldMs > 0f)
            {
                yield return WaitMilliseconds(profile.land.impactHoldMs);
            }

            yield return Animate(slideDurationMs, "linear", (eased, raw) =>
            {
                float slideProgress = FrictionSlideProgress(raw);
                Vector2 scale;
                if (raw < 0.48f)
                {
                    float k = EvaluateEase("easeOutCubic", raw / 0.48f);
                    scale = new Vector2(
                ...
                {
                    float k = EvaluateEase("easeOutCubic", (raw - 0.48f) / 0.52f);
                    scale = new Vector2(
                ...
                SetTransform(
                    card,
                    Vector2.LerpUnclamped(landStart, settledTarget, slideProgress),
                    Mathf.LerpAngle(landStartRotation, settledRotation, slideProgress),
                    deformationVisual != null ? Vector2.one : scale);
            });
            ...
            float landDurationMs)
        {
            ...
            // The requested distance is now the visible table slide. Only very short deals are
            // spatially capped; ordinary level and battle layouts receive the full configured value.
            float effectiveSlidePx = Mathf.Clamp(requestedSlidePx, 0f, totalDistance * 0.45f);
            ...
            // Keep some speed at contact so the locked compression frame reads as impact.
            // Timing still influences the incoming speed, but the slide deliberately restarts
            // after contact and decelerates under friction instead of preserving one continuous arc.
            float timingRatio = landDuration / (flightDuration + landDuration);
            float terminalProgressSlope = Mathf.Lerp(0.72f, 0.9f, Mathf.Clamp01(timingRatio));
            return new ImpactTransitionPlan
            {
                approach = approach,
                effectiveSlidePx = effectiveSlidePx,
                terminalProgressSlope = terminalProgressSlope,
                landingProgressSlope = 3f,
            };
        }

        /// <summary>Fast initial movement followed by a clear frictional stop.</summary>
        public static float FrictionSlideProgress(float raw)
        {
            float t = Mathf.Clamp01(raw);
        ...
        public static DustBurstSample SampleDustBurst(CardMotionProfile.EffectsSettings effects, string seed)
        {
